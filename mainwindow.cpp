...
{
    this->file_info.setFile(QFileDialog::getOpenFileName(this, "Open file", this->file_info.filePath(), "Binary Files (*.bin)"));
    if (this->file_info.exists()) {
        this->log("Sending from " + this->file_info.filePath());
        QFile file(this->file_info.filePath());
        if (!file.open(QIODevice::ReadOnly)) {
        ...
        this->Send(this->file_info.filePath(), true);
    }
}
...
void MainWindow::Receive()
{
    ...
    this->file_info.setFile(QFileDialog::getSaveFileName(this, "Save File", this->file_info.filePath(), "Binary Files (*.bin)"));
    if (this->file_info.exists()) {
        this->log("Saving to " + this->file_info.filePath());
        QFile file(this->file_info.filePath());
        if (!file.open(QIODevice::ReadWrite)) {
            qCritical("Could not save the file.");
            return;
        }
        this->Receive(this->file_info.filePath());
    }
}
...
void MainWindow::on_b_sendLast_clicked()
{
    if (!this->file_info.exists()) {
        QMessageBox::warning(this, tr("No file"), tr("No file sent yet. Use the \"Send...\"-button instead."));
        return;
    }
    ...
    if (this->file_info.exists()) {
        ...
        QFile file(this->file_info.filePath());
        if (!file.open(QIODevice::ReadOnly)) {
            QMessageBox::warning(this, tr("File problem"), tr("Could not open the file."));
            return;
        }
        this->log("Size: "+QString::number(file.size()/1024)+"KB");
